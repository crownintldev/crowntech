  ...
  {Aboutcore && Aboutcore.service.map((array, index) => (
    <motion.div  key={index} className={`shadow-lg group mb-8 rounded-lg max-h-full h-auto flex justify-center items-center relative  bg-primary-blue400 hover:bg-primary-blue300 text-primary-black  dark:bg-primary-white`} style={{
      ...
      <div className="p-4 rounded-lg group-hover:opacity-25">
        ...
        <HeadingH5 className="pl-2 text-primary-black" title={array.title} />
        <Para16 className="pl-2" title={array.para} />
      </div>
      
      <div className={`absolute opacity-0 w-full h-auto mt-10 mb-10 flex flex-col p-5 rounded-lg  group-hover:opacity-100 bg-primary-blue300 hover:bg-primary-blue300  dark:bg-primary-white text-primary-black`}>
        {array.item.map((items, index) => (
          <div className="flex flex-col gap-3" key={index}>
            <div className="flex">
              <TiTick size={22} className={``} />
              <Para16 className='text-primary-black' title={items.role} />
            </div>
          </div>
        ))}
        <Link className="mt-3" href={`${array.ref}`}>
          <Button className="bg-primary-blue300 hover:bg-primary-blue400 text-primary-black h-10" text="View More" />
        </Link>
      </div>
    </motion.div>
  ))}
